...
import { Users, Monitor, CheckCircle, XCircle } from 'lucide-react';
...
export default function Admin() {
  const { getAllUsers, updateUserRole, toggleUserStatus, approveDevice, denyDevice, getAllDevices, user } = useAuth();
  ...
  const refresh = () => {
    ...
    setLogs(getAuditLogs().slice(0, 50));
  };
  ...
  const handleRoleChange = (userId: string, role: string) => {
    ...
    updateUserRole(userId, role as Role);
    refresh();
  ...
  const tabs = [
    ...
    { key: 'logs', label: 'Audit Logs', icon: CheckCircle },
  ] as const;
                  ...
                  <tr key={u.id} className="border-b border-border last:border-0">
                    ...
                    <td className="p-3">
                      <Select value={u.role} onValueChange={v => handleRoleChange(u.id, v)}>
                        ...
                        <SelectContent>
                          {['USER','ADMIN','IT','SUPERADMIN'].map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-lg border border-border bg-card overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {devices.length === 0 && <tr><td colSpan={5} className="p-6 text-center text-muted-foreground">No devices registered yet</td></tr>}
              </tbody>
            </table>
          </motion.div>
        )}
              ...
              <tbody>
                {logs.map(l => (
                  <tr key={l.id} className="border-b border-border last:border-0 text-xs">
                    <td className="p-3 font-mono text-muted-foreground">{new Date(l.timestamp).toLocaleString()}</td>
                    <td className="p-3 text-primary font-mono">{l.userEmail || l.userId}</td>
                    <td className="p-3 text-foreground">{l.action}</td>
                    <td className="p-3 font-mono">{l.riskScore !== undefined ? l.riskScore : '—'}</td>
                ...
                {logs.length === 0 && <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">No audit logs yet</td></tr>}
              </tbody>
            </table>
          </motion.div>
        )}
